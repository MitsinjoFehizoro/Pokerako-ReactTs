        ...
        <div className="fixed bottom-0 left-0 right-0 z-50 p-4 w-full flex flex-col items-center justify-center">
            {
                toasts.map((toast, index) => {
                    ...
                    return (
                        <div key={index} className={`border-${toast.type} relative mb-2 min-w-80 py-2 flex items-center bg-white rounded-sm border-l-8`}>
                            <div className={`text-${toast.type} text-2xl px-4`}>
                                <i className={`fa fa-${icon}`}></i>
                            </div>
                            <div>
                                <h6 className="text-tertiary/90 font-semibold capitalize">{toast.type}</h6>
                                <p className="text-xs mt-[-.3em] text-tertiary/80">{toast.toast}</p>
                            </div>
